...
const menuItems_post = defineEventHandler(async (event) => {
  ...
  const id = randomUUID();
  db.prepare("INSERT INTO menu_items (id, name, category, price, image_url, is_available, branch_id,, wholesale_price) VALUES (?, ?, ?, ?, ?, 1, ?, ?)").run(id, name, category || "silog", Number(price), imageUrl, branchId, wholesalePriceValue);
  return {
    success: true,
