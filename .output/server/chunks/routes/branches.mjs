...
import { a as db, h as hasKitchen } from '../nitro/nitro.mjs';
import 'node:http';
import 'node:https';
import 'node:events';
import 'node:buffer';
...
router.post("/", (req, res) => {
  ...
  try {
    ...
    const insertUser = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `);
    const createAll = db.transaction((rows) => {
      for (const b of rows) {
        ...
        const createKitchen = b.has_kitchen !== void 0 ? b.has_kitchen : hasKitchen(businessType);
